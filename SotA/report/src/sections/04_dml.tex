...
\label{subsec:basics}
The high-dimensional setting of conflict data, where the number of potential confounders is large relative to the number of observations, necessitates the usage of ML methods.
...
\textcite{lewis_doubledebiased_2021} (alternative/comprehensive version: \cite{lewis_doubledebiased_2021-1}) were among the first researchers to introduce a solution for DML with Dynamic Treatment Effects (DTE), which focuses on dealing with temporal persistence and feedback loops (see \Cref{subsec:spatial_dynamic_confounding});
...
Nevertheless, applying the frameork to empirical settings like conflict research remains challenging due to multiple factors.
For instance, even after easing the assumption to sequential conditional exogenity, conflict settings might still violate it since unobversed strategic factors that drive both treatment and outcome (spatial confounding) are often present.
The model also does not take spatial spillovers into account, thereby violating the SUTVA essential for valid causal inference.

\subsection{DML for Heterogenous Treatment Effects in Dynamic Panels}
\label{subsec:dml_hte}
The frameworks presented in \cref{subsec:basics} and \cref{subsec:dml_dte} estimate the averge treatment effect (ATE)\footnote{For both HTE estimation exists. See \cite{semenova_debiased_2021} for the DML CATE extension and the original DML for DTE paper \cite{lewis_doubledebiased_2021}.}.
For policy applications or to analyze conflict diffusion, understanding how treatment effects vary throuhout contexts and characteristics is important.
While the dynamic DML frework (\Cref{subsec:dml_dte}) addresses temporal feedback loops, coflict data experpts additional challenges (See \cref{sec:spatio_temporal}).
...
Their method estimates the Conditional Average Treatment Effect (CATE)---which captuers treatment effect heterogenity as a function of unit characteristics---and works with high-dimensional panels; they specifically address static spatial confounding (unobserved unit-specific heterogenity/unit fixed effects) and statistical dependence (weak dependence).
...
The authors therefore take a fixed effects approcach, in which tehy approximate the vector of unobversed components of units effets $\xi^E$ by a weakly sparse vector.
Instead of treating the effects as a nuisance to be differenced out, the methods estimates them using Lasso.
This assumes that the unit-specific deviations are well-approximated by a sparse vector, allowing them to be learned consistently alongside other nuisance parameters even when number of units is large.

For the estimation oft the high-dimensional parameter $\theta_0$, the DML principle of orthogonalization is applied to partial out the influence of confounders and unit effects.
...
\end{align*}
with $\xi$ denoting a fixed vector of unit-specific fixed treatmentselection effects.
...
\textcite{semenova_inference_2023} resolve this with Nighbours-Left-Out (NLO) cross-fitting, where the time series is partitioned into $K$ adjacent bolcks $\{M_k\}_{k=1}^K$.
Then temporal neighbors are excluded by defining a new quasi-complement of for each block as $M_k^{qc} = \{M_1, \ldots, M_K\} \setminus \{M_l : l \in \mathcal{N}(k)\}$, where $\mathcal{N}(k)$ denotes $k$ and its immediate neighbors.
...
\begin{enumerate}
\item \textbf{Nuisance Estimation:} Estimate the nuisance functions $\hat{g}$ and $\hat{m}$ using appropritae modeling structures and ML methods that can handle fixed effects (for instance lasso for the sparse fixed effects) as well as NLO cross-fitting.
Afterwards, calculate the residuals $\tilde{Y}_{it}$ and $V_{it}$.
\item \textbf{Orthogonal Estimation:} Estimate the causal effect parameter vector (the CATE function) $\hat{\theta_0}$ by regressing $\tilde{Y}_{it}$ on $V_{it}$ using Lasso:
    \begin{equation*}
        \hat{\theta} = \arg\min_{\theta \in \mathbb{R}^d} \frac{1}{NT} \sum_{i,t} \left( \tilde{Y}_{it} - V_{it}' \theta \right)^2 + \lambda_\theta \|\theta\|_1.
    \end{equation*}
with $\lambda_\theta=C_\theta\sqrt{\log d/NT}$ and $C_\theta$ a penalty parameter.
...
The framework also does not explicitly address spatial spillovers, which is essential for researching conflict (\cref{subsec:sutva}).
...
\textcite{bodory_evaluating_2022} introduce introduce a weighting based DML extension strategy for estimating dynamic treatment effects, that robustly adjust for time-varying confounding across treatment sequences.
