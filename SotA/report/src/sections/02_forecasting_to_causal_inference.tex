...
A multitude of methodological approaches for effect estimation in CML have emerged, with recent research moving beyond the global Average Treatment Effect (ATE) and predominately focusing on Heterogeneous Treatment Effects (HTE) via the Conditional Average Treatment Effect (CATE) \cite{kunzel_metalearners_2019, athey_recursive_2016}.
The following taxonomy gives a broad overview of the major areas\footnote{Note that these categories are not mutually exlusive and conceptually different things. For instance DML can be seen as a statistical principle (bias-correction) that is implemented within specific algorithms (e.g., causal forests, R-learner DR-learner), while meta-learners can be seen as architectural wrappers (model composition).}:
\textbf{Tree-based methods}, most notably Causal Forests, are specialized and adapted versions of the random forest algorithn that split to maximize treatment effect heterogeneity across leaves, as opposed to prediction accuracy \cite{wager_estimation_2017, athey_generalized_2019}.
...
For instance the T-learner (T for Two) fits separate base-learners for treatment and control units, followed by taking the difference of model estimates; other variants include the S-learner or the X-learner \cite{kunzel_metalearners_2019, chen_recent_2025, chernozhukov_applied_2024}.
Double Machine Learning represents a general framework within the larger group of \textbf{Orthogonalization Methods}, that utilizes neyman-orthogonal score functions to control for regulatization bias in high-dimensional settings (For an extensive discussion see \cref{sec:dml}) \cite{chernozhukov_doubledebiased_2018}.
Furthermore extensions to Instrumental Variables (IV) and Difference-in-Differences (DiD) have recently come foreward to adress situations where the uncofoundedness assumption is violated \cite{chang_doubledebiased_2020, chernozhukov_doubledebiased_2018, lan_meta-learner_2025}.

Within conflict research CML has only received sparse application.
Notable exceptions include: \textcite{christiansen_toward_2022}, who applied spatio-temporal models to analyze the relationship between armed conflict and forest loss in Colombia; \textcite{kuzmanovic_causal_2024} with their CML framework for predictig HTEs of aid disbursements; or causal inference to evaulate the impart US airstrikec on insurgent voilence in Iraq \cite{papadogeorgou_causal_2022, mukaigawara_spatiotemporal_2025}. See also \cite{zhou_estimating_2025, sun_revealing_2022, ge_modelling_2022}.
However, a solutio to address the challenges of spatio-temporal diffusion in grid-call data used by \textcite{racek_capturing_2025} and applying the DML framework remains an open research gap.
